...
from base_datos_x_5 import traer_productos # base de datos externa en base_datos.py
import keyboard
from colorama import init
from colorama import Fore, Back, Style
from logo import mostrar_logotipo
from gracias import monty_chau, monty_cancelado
import os
    ...
    while True and medio_pago == "0":
        ...
        while True:
            if keyboard.is_pressed('t'):
                clean_terminal()
                print(Back.LIGHTBLUE_EX + f" ¿ Desea pagar con tarjeta de credito ? ")
                print(f" Presione S para Sí / N para No")
                medio_pago = "t"
                break
            if keyboard.is_pressed('d'):
                clean_terminal()
                print(Back.LIGHTBLUE_EX + f" ¿ Desea pagar con tarjeta de débito ? ")
                print(f" Presione S para Sí / N para No")                
                medio_pago = "d"
                break
            if keyboard.is_pressed('q'):
                clean_terminal()
                print(Back.LIGHTBLUE_EX + f" ¿ Desea pagar con QR/APP ? ")
                print(f" Presione S para Sí / N para No")                
                medio_pago = "q"
                break
...
def main():
    ...
    while indice_p < len(producto): # loop recorrido diccionario de productos
        ...
        print(Fore.BLACK + Back.LIGHTMAGENTA_EX + "\033[1m" + "┌" + " " * 34 + "AGREGE PRODUCTOS A SU PEDIDO   " + " "  * 35 + "┐")  # │ ─ ┌ ┐ └ ┘ ^ v  ˄˅
        ...
        while True: # while teclado
            try:
                ...
                break # ante cualquier otro error sale del while[Microsoft.PowerShell.PSConsoleReadLine]::ClearHistory()
